...
</div></section><section class="home-pets"><div class="actions" style="margin-bottom:32px"><RouterLink v-for="category in data.categories" :key="category.code" :to="`/pets?category=${category.code}`" class="btn secondary">认识{{category.name}} →</RouterLink></div><div class="section-heading"><div><p class="eyebrow">MEET YOUR COMPANION</p><h2>每一只，都值得认真认识。</h2></div><RouterLink to="/pets" class="text-link">查看全部 →</RouterLink></div><div v-if="featuredPets.length" class="pet-grid"><PetCard v-for="pet in featuredPets.slice(0,4)" :key="pet.id" :pet="pet"/></div><div v-else class="home-empty"><div><h3>下一次相遇，值得期待。</h3><p>目前暂无可展示的在售宠物。可以先了解门店，联系店主咨询。</p></div><RouterLink to="/shop" class="btn secondary">了解门店 →</RouterLink></div></section><section v-if="data.recommendedPets.length&&data.latestPets.length" class="home-pets"><div class="section-heading"><div><p class="eyebrow">NEW LITTLE FRIENDS</p><h2>新来的小伙伴</h2></div><RouterLink to="/pets" class="text-link">发现更多 →</RouterLink></div><div class="pet-grid"><PetCard v-for="pet in data.latestPets.slice(0,4)" :key="pet.id" :pet="pet"/></div></section><ShopCard :shop="data.shop"/></template></div></template>
