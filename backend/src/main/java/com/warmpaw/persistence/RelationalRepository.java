...
public class RelationalRepository {
  ...
  public List<Map<String,Object>> models() { return models; }
  private Map<String,Object> model(String kind) {
    return models.stream().filter(m -> kind.equals(m.get("kind"))).findFirst()
        .orElseThrow(() -> new IllegalArgumentException("Unknown entity kind: " + kind));
  }
  public String kindOf(String id) {
    return models.stream().map(m -> text(m,"kind")).sorted(Comparator.comparingInt(String::length).reversed())
        .filter(k -> id.startsWith(k + "_")).findFirst()
        .orElseThrow(() -> new IllegalArgumentException("Unknown entity id prefix"));
  }

  public Map<String,Object> find(String kind,String id) { return one(model(kind),"id",id); }
  public Map<String,Object> byKey(String kind,String key) { return one(model(kind),"business_key",key); }
  private Map<String,Object> one(Map<String,Object> m,String column,String value) {
    var rows=jdbc.queryForList("SELECT * FROM " + m.get("table") + " WHERE " + column + "=?",value);
    return rows.isEmpty()?null:hydrate(m,rows.getFirst(),text(rows.getFirst(),"id"));
  }
  public List<Map<String,Object>> list(String kind) {
    var m=model(kind);
    return jdbc.queryForList("SELECT * FROM " + m.get("table") + " ORDER BY created_at DESC,id DESC")
        .stream().map(row -> hydrate(m,row,text(row,"id"))).toList();
  }
  ...
  private Map<String,Object> columns(Map<String,Object> m,Map<String,Object> entity) {
    ...
    for (var f:objects(m,"fields")) result.put(text(f,"column"),value(f,entity.get(text(f,"field"))));
    return result;
  }
  ...
  private Object decode(Map<String,Object> f,Object value) {
    ...
    if ("BOOLEAN".equals(f.get("sql"))) return value instanceof Boolean ? value : ((Number)value).intValue()!=0;
    return value;
  }
  private Map<String,Object> hydrate(Map<String,Object> m,Map<String,Object> row,String id) {
    ...
    for (var c:objects(m,"children")) {
      ...
      var rows=jdbc.queryForList("SELECT * FROM " + c.get("table") + " WHERE parent_id=?"+(list?" ORDER BY position":""),id);
      if (list) {
  ...
  private void validate(Map<String,Object> m,Map<String,Object> entity) {
    Set<String> allowed=objects(m,"fields").stream().map(f -> text(f,"field")).collect(Collectors.toSet());
    for (var c:objects(m,"children")) {
      if ("embedded".equals(c.get("mode"))) {
        var embedded=new LinkedHashMap<String,Object>();
        for(var f:objects(c,"fields")) {String key=text(f,"field");allowed.add(key);if(entity.containsKey(key))embedded.put(key,entity.get(key));}
        validate(c,embedded);
      } else {
        String path=text(c,"path");allowed.add(path);
    ...
    for(String key:entity.keySet()) if(!allowed.contains(key)&&entity.get(key)!=null)
  ...
  public void insert(String kind,String key,Map<String,Object> entity) {
    var m=model(kind);validate(m,entity);
    var row=columns(m,entity);row.put("business_key",key);
    insertRow(text(m,"table"),row);writeChildren(m,entity,text(entity,"id"));
  }
  @Transactional
  public void update(Map<String,Object> entity) {
    var m=model(kindOf(text(entity,"id")));validate(m,entity);
    var row=columns(m,entity);row.remove("id");
    List<Object> values=new ArrayList<>(row.values());values.add(entity.get("id"));
    int n=jdbc.update("UPDATE " + m.get("table") + " SET " + row.keySet().stream().map(k -> k+"=?").collect(Collectors.joining(","))+" WHERE id=?",values.toArray());
    if(n!=1)throw new IllegalStateException("Entity update did not match one row");
    // 主表与一对多关系在同一事务替换，任何附件约束失败都会回滚。
    deleteChildren(m,text(entity,"id"));writeChildren(m,entity,text(entity,"id"));
  }
    ...
    for(var c:objects(m,"children")) {
      ...
      if("list".equals(mode)) {
        ...
        for(int i=0;i<list.size();i++) {
          @SuppressWarnings("unchecked") Map<String,Object> item=c.get("scalar")==null?(Map<String,Object>)list.get(i):map(text(c,"scalar"),list.get(i));
          var row=columns(c,item);row.put("parent_id",id);row.put("position",i);insertRow(text(c,"table"),row);
        }
      } else if("embedded".equals(mode)||entity.get(path)!=null) {
        var item="embedded".equals(mode)?entity:object(entity,path);
        var row=columns(c,item);row.put("parent_id",id);insertRow(text(c,"table"),row);
