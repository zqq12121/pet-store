...
1. 备份数据库，依次执行同目录 `appointment-migration.sql` 和 `appointment-sms-outbox-migration.sql`。脚本只新增预约子表和短信 outbox，可重复建表，不清空或重写原订单。生产配置禁止启动时自动建表，不能跳过此步骤。
...
线下收款标记为 `offline_received`，没有微信流水；工作台将它计入收款总额。线下预约的退款、售后由门店联系处理，不进入微信自动原路退款。

预约提交、确认、取消、过期和交付完成会在业务事务内登记短信 outbox，提交后发送，失败自动重试，最多八次。真实发送需要 `PAW_SMS_SIGN_NAME` 和对应的 `PAW_SMS_APPOINTMENT_*_TEMPLATE`；缺失配置会记录失败并重试，不代表已发送。本地模拟模式仅写测试收件箱。平台受理不等于手机送达；平台已受理但本地成功状态尚未落库时若进程中断，重试可能重复发送。当前没有微信通知，店长仍可在工作台和预约列表查看、刷新处理。
